...
export class AuthService {
  ...
  register(credentials: Credentials): Observable<AuthUser> {
    return this.http
      .post<AuthUser>("http://localhost:8093/api/auth/register", credentials)
  ...
  login(credentials: Credentials): Observable<AuthUser> {
    return this.http
      .post<AuthUser>("http://localhost:8093/api/auth/login", credentials)
      .pipe(tap((user) => this.setUser(user)));
  }
   ...
   */
  logout(): void {
    localStorage.removeItem("enterprise_store_user");
  ...
  private setUser(user: AuthUser): void {
    localStorage.setItem("enterprise_store_user", JSON.stringify(user));
  ...
  private readUser(): AuthUser | null {
    try {
      return JSON.parse(
        localStorage.getItem("enterprise_store_user") ?? "null",
      ) as AuthUser | null;
    } catch {
      return null;
    }
